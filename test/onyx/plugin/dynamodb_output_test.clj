(ns onyx.plugin.dynamodb-output-test
  (:require [clojure.core.async :refer [chan >!! <!! close! sliding-buffer]]
            [clojure.test :refer [deftest is testing]]
            [taoensso.timbre :refer [info]]
            [onyx.plugin.core-async]
            [onyx.plugin.dynamodb-output]
            [onyx.api]))
...
(def catalog
  [{:onyx/name :in
   ...
   {:onyx/name :out
    :onyx/plugin :onyx.plugin.dynamodb-output/output
    :onyx/type :output
...
(defn inject-out-datasink [event lifecycle]
  {:dynamodb/example-datasink out-datasink})
...
(def lifecycles
  [{:lifecycle/task :in
   ...
   {:lifecycle/task :out
    :lifecycle/calls :onyx.plugin.dynamodb-output/writer-calls}])
