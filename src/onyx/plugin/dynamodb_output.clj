(ns onyx.plugin.dynamodb-output
...
(defn inject-writer
  [event lifecycle]
  (when-not (:dynamodb/example-datasink event)
...
(defrecord ExampleOutput []
  ...
  (write-batch 
    ...
    [_ {:keys [onyx.core/results dynamodb/example-datasink] :as event}]
  ...
  (seal-resource 
    ...
    [_ {:keys [dynamodb/example-datasink]}]
